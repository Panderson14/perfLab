...
int tick(bool showpc=true, bool showall=false) {
    ...
    ulong _HCL_valP = (_HCL_P_pc)+(_HCL_offset);
    _HCL_valP &= 0xffffffffffffffff;
    if (showall) writefln("set valP to 0x%x",_HCL_valP);
    ulong _HCL_reg_srcA = ((((_HCL_icode)==(2))) ? (_HCL_rA) :
		(15));
    _HCL_reg_srcA &= 0xf;
    if (showall) writefln("set reg_srcA to 0x%x",_HCL_reg_srcA);
    ulong _HCL_reg_dstE = (((((_HCL_icode)==(3)))||(((_HCL_icode)==(2)))) ? (_HCL_rB) :
		(15));
    _HCL_reg_dstE &= 0xf;
...
pragma(msg,`Estimated clock delay: 67`);
enum tpt = 67;
