######### The PC #############
register xF { pc:64 = 0; }				# the register bank


########## Fetch #############

pc = F_pc;								# put the address of the next instruction into this, starts with 0x000.

wire icode:4, rA:4, rB:4, valC:64;				# set up the variables

icode = i10bytes[4..8];				# instruction memory output
rA = i10bytes[12..16];
rB = i10bytes[8..12];
...
valP = F_pc + offset;	# increments the pc
x_pc = valP;			# through our predefined register.
...
f_stat = [
	...
	1 : STAT_AOK;
];			# checks the current instruction to see if it is valid before doing anything with it. We don't check in any other stage because we have already checked in this one.


f_icode = icode;	# pipeline the current instruction pieces. No need to pipeline valP because we 						have already used it completely to increment the pc to the next 									instruction so it can begin pipelining the next instruction.
f_rA = rA;
f_rB = rB;
f_valC = valC;
...
register fD {
	stat:3 = STAT_BUB;	# STAT_BUB is simply the initial value 0b000, we set it earlier
	icode:4 = NOP;		# same with NOP
	rA:4 = REG_NONE;
	rB:4 = REG_NONE;
	valC:64 = 0;
}
...
d_dstM = [	# Because we are forwarding the value of rA if another piece of the code needs it from either memory or writeback.
...
# we don't save the rA and rB values because we need srcA and srcB for other instructions. We have successfully decoded them.
d_stat = D_stat;
d_icode = D_icode;
d_valC = D_valC;
